...
import React, { ChangeEvent, SyntheticEvent, useEffect, useId, useState } from "react";
import { OptionalTypeNode } from "typescript";
import { OutputImageCard } from "./OutputImageCard";
import { SRImagesAddresses, download, getSRFaces, getSRFaceAndScene, getSRScene, getFile } from "../service";
...
export function ImgCompareSlider({
  ...
  let src1 = secondImage instanceof File ? URL.createObjectURL(secondImage) : secondImage;
  let src2 = firstImage instanceof File ? URL.createObjectURL(firstImage) : firstImage;
...
export function FinalImageResult({
  ...
  useEffect(() => {
    ...
    if (beforFile)
      download(beforFile).then(u => setFirstUrl(u))
...
export default function Container() {
  ...
  const [status, setStatus] = useState<'normal' | 'processing' | 'downloading' | 'done'>('normal')
  const statusText =
    status === 'processing'
      ? 'در حال پردازش'
      : status === 'downloading'
  ...
  const handleSubmit = async () => {
    ...
    if (status === 'downloading' || status === 'processing') {
      alert(`تصویر از قبل ${statusText} است لطفا کمی صبر کنید یا در صورت تمایل عملیات مورد نظر را تغییر دهید.`)
      return;
    }
  ...
  return (
    <div dir="rtl" className="sr-container m-auto">
      <div className="flex flex-wrap justify-around">
        ...
        <Card className="w-full min-h-96 max-h-[400px] m-2 sm:w-3/5 md:w-2/6">
          <div className="flex w-full h-full justify-center items-center flex-wrap">
            ...
            <div className="w-11/12 my-2">
              <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-white" htmlFor={fileInputId}>انتخاب عکس</label>
              <input
                ref={fileEl}
                id={fileInputId}
            ...
            <div className="w-11/12 my-2">
              ...
              <div className="w-full min-w-[200px]">
                <div className="relative">
                  <select value={srType} onChange={handleChangeSrType}
                  ...
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.2" stroke="currentColor" className="h-5 w-5 ml-1 absolute top-2.5 left-2.5 text-slate-700">
            ...
            <div className="flex w-11/12 my-2 flex-wrap items-center justify-around">
              ...
              <div className="sr-card-field w-full flex items-center p-2">
                ...
                <div className="relative mb-6 w-full">
                  {/* <label htmlFor={fidelityElementId} className="sr-only">Labels range</label> */}
                  <input id={fidelityElementId} type="range"
                  ...
                  />
                  <span className="text-sm text-gray-600 absolute start-0 -bottom-5">کیفیت</span>
                  <span className="text-sm text-gray-600 absolute end-0 -bottom-5">وفاداری</span>
                </div>
              </div>
            </div>
            ...
            {!imagePaths?.zipFile ? null : (
              <div className="w-11/12 my-2">
                <button
                  ...
                  <span className="w-full relative px-5 py-2.5 transition-all ease-in duration-75 bg-white dark:bg-gray-900 rounded-md group-hover:bg-transparent group-hover:dark:bg-transparent">
                    <svg className="inline fill-current w-4 h-4 mx-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M13 8V2H7v6H2l8 8 8-8h-5zM0 18h20v2H0v-2z" /></svg>
                    دانلود همه تصاویر
                  </span>
                </button>
        ...
        <FinalImageResult beforFile={image} afterFile={imagePaths?.final} onResultLoad={() => setStatus('done')} />
